...
def speech_to_text():
    ...
    f = open('audiobooks/'+filename_with_json)
    data = json.load(f)
    audible_key=data["key"]
    audible_iv=data["iv"]

    process = subprocess.Popen(['ffmpeg', '-audible_key', audible_key, '-audible_iv', audible_iv,'-loglevel', 'quiet', '-i',
                                'audiobooks/'+filename_with_extension,
                                # sys.argv[1],
